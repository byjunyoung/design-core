...
// variant axis in the order the axes are declared, then the state's patches, then the
// breakpoint's. Variants are what a screen *is* for this record or mode (an edit modal for a
// counted item, a dialog in Edit mode); states are what it is *doing* (Empty, Loading); a
// breakpoint is how wide the viewport is (conventions.breakpoints), which is why it comes
// last — the same state, narrower. The merged view is computed here and never stored (§3).
export function mergeState(screen, state, variants = {}, breakpoint = null) {
  ...
  if (breakpoint) applyPatches(screen.breakpoints?.[breakpoint] ?? [], elements, layout, missingTargets);

  return { state, variants, breakpoint, elements, layout, missingTargets };
}
