...
test('the server exposes the verbs and the two agent reads', async () => {
  ...
  assert.deepEqual(tools.map((t) => t.name).sort(), ['diff', 'get_screen', 'lint', 'list_missing', 'list_components', 'list_flows', 'list_screens', 'list_tokens', 'list_assets', 'prep', 'render', 'propose', 'list_proposals', 'apply', 'reject', 'undo', 'import_figma', 'map_figma', 'list_comments', 'resolve_comment', 'add_comment'].sort());
});
