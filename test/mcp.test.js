...
test('the server exposes the verbs and the two agent reads', async () => {
  ...
  assert.deepEqual(tools.map((t) => t.name).sort(), ['diff', 'get_screen', 'lint', 'list_missing', 'list_screens', 'prep', 'render', 'propose', 'list_proposals', 'apply', 'reject', 'undo', 'import_figma'].sort());
});
